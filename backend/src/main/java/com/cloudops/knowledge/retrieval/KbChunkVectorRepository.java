...
public class KbChunkVectorRepository {
    ...
    public void insertChunk(
            ...
            String metadataJson,
            float[] embedding) {
        String vector = com.cloudops.knowledge.indexing.VectorUtils.toPgVector(embedding);
        jdbcTemplate.update(
                """
    ...
    public List<ScoredChunk> searchSimilar(float[] queryEmbedding, int topK, double minSimilarity) {
        String vector = com.cloudops.knowledge.indexing.VectorUtils.toPgVector(queryEmbedding);
