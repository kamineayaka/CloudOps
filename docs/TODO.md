...
## P2 — 运维内置工具（平台 `AgentTool`）

当前仅 2 个内置工具：`list_assets`、`ssh_exec`（`backend/src/main/java/com/archops/tools/`）。  
AI Agent 通过 `ToolRegistry` 注册，经 `ToolExecutorService` 走风险分级与审批门控。  
说明：这是进程内工具注册表，并非 MCP 协议实现；外部 MCP Server 见 P3。
...
1. 每个工具实现 `AgentTool` 接口，加 `@Component`，自动注册。
...
| # | 任务 | 说明 |
|---|------|------|
| P2-B1 | 新增 `docs/agent-tools.md` | 列出所有内置工具、参数、风险等级、审批策略 |
...
ArchOps **平台内** Agent 使用 Java `AgentTool`（进程内注册表）；**开发/运维同学在 Cursor IDE** 中可额外接入标准 MCP Server，加速日常排障与批量操作。  
...
| MCP | 场景 | 来源建议 | 优先级 |
|-----|------|----------|--------|
| **SSH** | 远程命令、文件读写、批量巡检 | 已有 `user-ssh` 或自研 `archops-ssh-mcp` 对接平台资产 API | 高 |
...
| **ArchOps API** | 自研 MCP：调平台 REST（资产、审批、知识库、AI） | **建议自研** `mcp-archops`，统一认证 | 高 |

### P3 自研 `mcp-archops` 建议能力

```
tools:
  - archops_list_assets
  - archops_ssh_exec          # 代理到平台 API，继承 RBAC + 审批
  - archops_search_knowledge
  - archops_create_approval
  - archops_chat              # 可选：把自然语言转平台 Agent
```

**技术选型：** TypeScript `@modelcontextprotocol/sdk` 或 Python `mcp`，读 `ARCHOPS_URL` + `ARCHOPS_TOKEN`。

| # | 任务 | 说明 |
|---|------|------|
| P3-1 | 脚手架 `tools/mcp-archops/` | package.json / README / 最小 `list_assets` |
| P3-2 | `docs/mcp-cursor-setup.md` | `.cursor/mcp.json` 示例（SSH + archops） |
...
| P4-4 | 可观测性栈默认 Dashboard | Grafana 预置 ArchOps 面板 |
...
当前迭代重点：P0 测试 + P2 内置工具（先做 get_asset、knowledge_search、batch_ssh_exec）。
