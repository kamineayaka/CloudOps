...
| **ML-2** | 分区键与修订可读；事实带 provenance 约束 | [x] |
| **ML-3** | A3–A5、B3（提案→合并→回滚） | [x] |
| **ML-4** | A2–A3（Classifier + propose 工具） | [x] |
| **ML-5** | B2 Work Log 按会话可查；晋升仅经 Proposal | [x] |
| **ML-6** | B1 范围化 RAG；非整本 summary 唯一策略 | [x] |
| **ML-7** | C2、C3 与资产范围 ACL | [x] |
| **ML-8** | 架构浏览器 + 提案台可走完 A3–A5 | [x] |
