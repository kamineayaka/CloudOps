...
当前仅 2 个内置工具：`list_assets`、`ssh_exec`（`backend/src/main/java/com/cloudops/tools/`）。  
...
CloudOps **平台内** Agent 使用 Java `AgentTool`（进程内注册表）；**开发/运维同学在 Cursor IDE** 中可额外接入标准 MCP Server，加速日常排障与批量操作。  
